...
class Article
{
    ...
    public static function create($title, $abstract, $text, $user_id)
    {
        ...
        $query = "
            INSERT INTO articles (title, abstract, text, user_id)
    ...
    public static function countByUser($user_id)
    {
        ...
        $query = "SELECT COUNT(*) as total FROM articles WHERE user_id = '$user_id'";
        $res = $db->query($query);
    ...
    {
        $db = Db::getInstance();
        $query = "DELETE FROM articles WHERE id = $this->id LIMIT 1;";
        return $db->query($query);
    }
}
